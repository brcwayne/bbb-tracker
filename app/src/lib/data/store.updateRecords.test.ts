...
import { updateRecords, deleteRecords } from './store'
...
describe('deleteRecords', () => {
  it('eşleşen tüm satırları tek seferde siler, eşleşmeyenleri korur', async () => {
    const dataset: any = {
      transactions: [], cashflows: [], snapshots: [], instruments: [], brokers: [],
      portfolios: [], meta: {}, fxrates: {}, assetTransfers: [],
      personalTx: [
    ...
    await deleteRecords(
      store, source, 'personal_tx',
      (r: any) => r.tekrarKuralId === 'rr_1' && r.durum === 'planlandi',
    )
    const rows = get(store).dataset?.personalTx ?? []
    expect(rows.map((r: any) => r.id)).toEqual(['px_3']) // sadece onaylanmış geçmiş satır kaldı
  })

  it('hiçbiri eşleşmezse dosyayı olduğu gibi korur', async () => {
    const dataset: any = {
      transactions: [], cashflows: [], snapshots: [], instruments: [], brokers: [],
      portfolios: [], meta: {}, fxrates: {}, assetTransfers: [],
      personalTx: [
        { id: 'px_1', tarih: '2026-09-05', tur: 'GIDER', tutar: 100, paraBirimi: 'TRY', kategori: 'eglence', aciklama: '', hesap: 'nakit', sahip: 'enis', taksitPlaniId: null, taksitNo: null, taksitToplam: null, not: '', kaynak: 'manual', olusturulma: '' },
      ],
      paymentPlans: [], personalAccounts: [], categories: [], people: [], debts: [], recurringRules: [],
    }
    const store = writable<AppState>({ status: 'ready', dataset, derived: {} as any, sourceText: '' } as any)
    ...
    await deleteRecords(store, source, 'personal_tx', (r: any) => r.tekrarKuralId === 'rr_yok')
    const rows = get(store).dataset?.personalTx ?? []
    expect(rows.map((r: any) => r.id)).toEqual(['px_1'])
  })
})
