...
- **Faz 2 Düzenleme ve İki Yönlü Eşitleme:**
  - **Harcamalar:** `+ Harcama Ekle` butonuyla yeni harcama veya gelir kaydı girişi; her satırda inline `Düzenle` ve iki adımlı onay gerektiren `Sil` butonları. Taksitli bir harcama satırının plan bağı koparılamaz (plan bilgileri salt-okunur korunur).
  - **Taksitler:** Her aktif plan kartında `Planı İptal Et` butonu. İptal işlemi kaç gelecek taksitin silineceğini açıkça belirtip onay ister; onaylandığında plan kaydı (`payment_plans`) ve yalnızca gelecekteki taksitler (`tarih > bugün`) silinir, geçmiş taksitler (harcanmış para) korunur.
  - **Borçlar:** Açık kayıtlar tablosundan borç kapatma (`Kapat`, satırın durumu `KAPALI` yapılır, bakiye listesinden çıkar ancak geçmiş kaydı korunur), tutar/açıklama düzeltme (`Düzenle`) ve onaylı kalıcı silme (`Sil`).
  - **Kaynak Etiketleri (`kaynak`) ve Yetkilendirme:** Web arayüzünden eklenen satırlar `kaynak: 'manual'`, Telegram botundan eklenenler `kaynak: 'telegram'` olarak etiketlenir. Web tarafı `allowKaynak: ['telegram', 'manual']` ile her iki kaynaktan gelen kişisel kayıtları düzenleyebilir.
  - **Üç Yönlü Birleştirme (Three-way Merge):** Telegram botu periyodik senkronizasyonunda Drive'daki dosyaları ezmez (`overwrite` etmez); bot tarafındaki yerel kopya, Drive'daki güncel kopya ve son bilinen ortak temel (`.sync-base`) arasında üç yönlü satır bazlı birleştirme uygular. Web'de yapılan düzeltmeler ve silmeler ile Telegram'dan girilen yeni kayıtlar sessiz veri kaybı yaşanmadan uzlaştırılır. Ayrıntılar için bkz: `docs/superpowers/specs/2026-09-08-bbb-kisisel-defter-faz2.md`.
  - **Çakışma ve Çevrimdışı Koruması:** Eşzamanlı yazma durumunda (`ConflictError`), uygulama veriyi otomatik olarak yeniden yükler ve kullanıcıya *"Bu dosya başka bir yerden değişti, sayfa yenilendi — düzenlemeyi tekrar yapar mısın?"* uyarısı gösterir. Google Drive bağlantısı olmadığında (yerel geliştirme modunda) üç sayfadaki tüm düzenleme kontrolleri devre dışı bırakılır ve uyarı mesajı görüntülenir. Hiçbir yazma hatası sessizce yutulmaz.
